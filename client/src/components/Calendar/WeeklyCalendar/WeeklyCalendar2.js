...
const tasksEachDay = Array(days.length).fill().map(() => Array(hours.length));
const renderHeader = (today) => 
{
  ...
  if(tasksEachDay.length !== 0)
  {
    ...
    if(task)
            return <WeeklyCellTask key={task._id} task={task} click={()=> {
          setShowModal(true);
          setModalInfo(task);
        }}/>
        else
  ...
  useEffect(() => {
    ...
    //   orderTasksInArray()
    // );
    dispatch(getWeekTasksOfUser(moment(today.startOf("week").toDate()).format('YYYY-MM-DD'))).then(
      orderTasksInArray()
    );
  ...
  return (
    <>
        {/* {console.log(tasks)} */}
      {
       modalInfo !== null ? <WeeklyTaskModal show={showModal} 
       onClose={()=> {
        setShowModal(false);
        setModalInfo(null);
      }}
      ...
      <div className="WeeklyCalendar">
          ...
          <div className="calendar-header">
            <div className="calendar-navigate" onClick={() => lastWeek()}> &#10094; </div>
          <div>
          ...
          {hours.map((hour, hour_index) => (
            <tr key={hour_index}>
              ...
              <td key={`${hour_index},${day_index}`} id={`${hour}+${day_index}`}>
                {day_index === 0 ? hour : "" }
                {day_index > 0 && tasks.length? appendTasksToCell(day_index-1,hour_index) : "" }
